...
#コマンドライン引数からキーワードを得る
keyword = sys.argv[1] # 0番目は自身のスクリプト名
	
# カレントディレクトリ以下のファイルをすべて処理する
for root, dirs, files in os.walk("."):   # os.walkでファイルを列挙　https://docs.python.org/ja/3/library/os.html?highlight=os%20walk#os.walk
	for fi in files:
		result = [] # 結果を放り込む
		# テキストファイルを読む
		try:
			...
			with open(path,encoding = 'utf-8') as f: 
				for no,line in enumerate(f):  # ファイルの各行をインデックス番号で取得
